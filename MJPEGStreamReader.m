...
Action: file_editor create /app/vcam-patches/MJPEGStreamReader.m --file-text "// MJPEGStreamReader.m - VirtualCamPro V272.0 (Stage 1 fixes)
// Changes vs prior version:
//   * Added pixelBufferCallback - HLS path delivers CVPixelBufferRef directly
//     (no CIImage/UIImage/CGContext round-trip). ~5x cheaper per frame.
//   * MJPEG path also builds CVPixelBufferRef via CGImageSource + IOSurface-backed
//     buffer when pixelBufferCallback is set.
//   * Auto-reconnect for HLS on failed item status.
...
- (void)startHLSStream {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        self.hlsPlayer.automaticallyWaitsToMinimizeStalling = NO; // low-latency-ish
...
- (void)displayLinkCallback:(CADisplayLink *)sender {
    ...
    // PREFERRED: deliver the pixel buffer directly to the tweak.
    if (self.pixelBufferCallback) {
        self.pixelBufferCallback(pixelBuffer); // ownership transferred
        return;
    }

    // Legacy UIImage path (kept for compatibility - heavy).
    ...
    if (image && self.frameCallback) self.frameCallback(image);
}
    ...
    if (sRange.location != NSNotFound && eRange.location != NSNotFound &&
        ...
        if (self.pixelBufferCallback) {
            ...
            if (pb) {
                self->_frameCount++;
                self->_lastFrameTime = CFAbsoluteTimeGetCurrent();
                self.pixelBufferCallback(pb); // ownership transferred
            }
...
- (void)URLSession:(NSURLSession *)session task:(NSURLSessionTask *)task
    ...
    if (error) {
        ...
        if (self.errorCallback) {
            dispatch_async(dispatch_get_main_queue(), ^{ self.errorCallback(error); });
        }
        if (self.isRunning) {
...
@end
"
Observation: Create successful: /app/vcam-patches/MJPEGStreamReader.m
