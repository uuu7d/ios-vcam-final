// MJPEGStreamReader.m - VirtualCamPro V272.0 (Stage 1 fixes)
...
- (void)startStreaming {
    if (_isRunning) { NSLog(@"[VCamStream] Already streaming"); return; }
    _isRunning = YES;
    _isConnecting = YES;
...
- (void)startHLSStream {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(playerItemDidReachEnd:)
            name:AVPlayerItemDidPlayToEndTimeNotification object:self.hlsPlayerItem];
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(playerItemFailed:)
            name:AVPlayerItemFailedToPlayToEndTimeNotification object:self.hlsPlayerItem];
    });
}
